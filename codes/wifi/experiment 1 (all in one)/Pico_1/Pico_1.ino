...
void setup() {
  ...
  if (waitForESP32Boot()) {
    ...
    Serial.println("[Pico] WARNING: ESP32 boot timeout.");
  }
}
...
void loop() {
  ...
  for (int i = 0; i < NUM_PAYLOADS; i++) {
    ...
    for (long sent = 0; sent < size; sent++) {
      Serial1.write((uint8_t)digit);
      if (sent % 256 == 0) delay(10);
    }
    ...
    if (esp32Response == "FAIL") {
      ...
      skipRun = true;
    } else if (esp32Response == "OK") {
      waitForAckOrSkip(skipRun);
    } else {
      Serial.print("ESP32_TIMEOUT ");
      Serial.println(size);
      waitForAckOrSkip(skipRun);
      skipRun = true;
    }
