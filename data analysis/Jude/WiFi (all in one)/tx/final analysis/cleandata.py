...
def process_file(filepath):
    ...
    # create sequential phase blocks
    df["block"] = (df["phase"] != df["phase"].shift()).cumsum()

    blocks = (
        df.groupby("block", sort=False)["phase"]
        .first()
        .reset_index()
    )
    blocks_to_remove = []
    found_tx1 = False
    idle_after_tx1_removed = False

    for idx, row in blocks.iterrows():
        block_id = row["block"]
        phase = row["phase"]
        # remove first idle after baseline
        if idx > 0:
            prev_phase = blocks.iloc[idx - 1]["phase"]

            if prev_phase == "baseline" and "idle" in phase:
        ...
        if phase == "tx_1":
            ...
            found_tx1 = True
            continue
        # remove idle immediately after tx_1
        if found_tx1 and not idle_after_tx1_removed:
            if "idle" in phase:
                blocks_to_remove.append(block_id)
                idle_after_tx1_removed = True

    # apply removal
    df = df[~df["block"].isin(blocks_to_remove)].copy()
