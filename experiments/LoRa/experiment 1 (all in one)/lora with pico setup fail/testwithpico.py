...
PICO_PORT      = "/dev/cu.usbmodem141301"
PICO_BAUD      = 115200

RX_PORT = "/dev/cu.usbmodem143201"  # <-- receiver RN2903
RX_BAUD = 57600
...
def force_idle(pico):
    """Put the TX module into true idle (standby) by stopping any RX/TX activity."""
    send_cmd(pico, "radio rxstop")
    time.sleep(0.1)  # small settle time for the module to stabilise
    ...
    while time.time() < deadline:
        if pico.in_waiting:
            ...
            if line == "ok":
                ...
                got_tx = True
                # ── FIX: immediately return module to idle after TX completes ──
                force_idle(pico)
                break
...
def run_experiment(run_number, meter, pico):
    ...
    # ── FIX: force module to idle before baseline measurement begins ──
    force_idle(pico)
...
if __name__ == "__main__":
    ...
    # ── FIX: ensure module starts in idle before any run begins ──
    force_idle(pico)
