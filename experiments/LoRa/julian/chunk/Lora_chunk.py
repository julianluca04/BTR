...
# Generates [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, ..., 65536]
TOTAL_SIZES = [2**i for i in range(17)] 
CHUNK_SIZE = 220
TOTAL_RUNS = 30
IDLE_S = 1.0        
BASELINE_S = 5.0    
...
OUT_DIR = os.path.join(SCRIPT_DIR, "data", "LoRa", "exponential_chunks", SESSION_TAG)
...
def run_one_experiment(run_num, meter, tx_ser):
    filename = os.path.join(OUT_DIR, f"LoRa_exp_run{run_num:02d}.csv")
    ...
    with open(filename, "w", newline="") as f:
        ...
        writer.writerow(["strategy", "exponential_chunking"])
        writer.writerow(["chunk_limit", CHUNK_SIZE])
        ...
        for total_bytes in TOTAL_SIZES:
            print(f"\n  [Run {run_num}] Total Message: {total_bytes} bytes")
            ...
            while bytes_sent < total_bytes:
                ...
                hex_payload = "AA" * current_chunk_size
                
                # Send chunk to module
                tx_ser.reset_input_buffer()
                tx_ser.write(f"radio tx {hex_payload}\r\n".encode())
                
                # Wait for Hardware Confirmation (radio_tx_ok)
