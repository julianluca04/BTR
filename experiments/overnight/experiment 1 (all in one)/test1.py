...
ESP32_SKETCH   = '/Users/foml/coding/MSP/year_3/BTR/experiments/overnight/experiment 1 (all in one)/wifi_1'
PICO_FQBN      = "rp2040:rp2040:rpipico"
PICO_SKETCH    = '/Users/foml/coding/MSP/year_3/BTR/experiments/overnight/experiment 1 (all in one)/Pico_1'
...
def run_experiment(run_number, attempt, meter, pico, pico_already_ready=False):
    """Returns (valid, pico_ready)."""
    filename = os.path.join(
        OUT_DIR, f"{MODULE}_{STRATEGY}_run{run_number:02d}_attempt{attempt:02d}.csv"
    )
    meter_rows = []
    event_rows = []
    ...
    try:
        if pico_already_ready:
            print(f"\n[Run {run_number:02d} Attempt {attempt}] Pico ready.")
        else:
            print(f"\n[Run {run_number:02d} Attempt {attempt}] Waiting for Pico READY...")
        ...
        print(f"\n─── Run {run_number:02d} Attempt {attempt} Results ──────────────────────────────────────")
        ...
        print(f"[✓] Run {run_number:02d} Attempt {attempt} complete. {passed}/{len(results)} passed.")
        print(f"    → {filename}")
