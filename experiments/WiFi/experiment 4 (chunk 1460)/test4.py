...
PAYLOAD_SIZES = [
    ...
    131072, 262144
    # 524288 and 1048576 excluded — ESP32-C3 heap fragmentation causes failures
    # above ~262KB after repeated WiFiClient allocations (hardware limitation)
]
...
SHUNT_OHMS = 1.13  # measured shunt value
V_SUPPLY   = 5.0   # ESP32 powered from Pico VBUS
        ...
        for i, payload_size in enumerate(PAYLOAD_SIZES):
            ...
            try:
                ...
                with conn:
                    ...
                    recv_to  = max(30, int(declared / 11520.0 * 3) + 30)
                    payload  = recv_exact(conn, declared, total_timeout=recv_to)
